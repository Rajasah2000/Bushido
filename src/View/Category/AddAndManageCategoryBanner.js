...
const AddAndManageCategoryBanner = () => {
  ...
  const onDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        HomeService.DeleteBannerImage(id)
          .then((res) => {
            if (res && res.status) {
  ...
  const AddCategoryBanner = () => {
    ...
    if (image) {
      HomeService.AddCategoryBanner(data)
        .then((res) => {
          if (res && res.status) {
      ...
      toast.error("Category banner field is required");
    }
  };
  ...
  const UpdateCategoryBanner = () => {
    ...
    if (image) {
      HomeService.UpdateBannerImage(id, data)
        .then((res) => {
          if (res && res.status) {
...
export default AddAndManageCategoryBanner;
